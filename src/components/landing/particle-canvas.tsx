    ...
    const draw = () => {
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < MAX) {
            ...
            const color  = isHot ? EMERALD : "51,65,85";
      ...
      for (const p of particles) {
        ...
        if (p.type === "hub") {
          ...
          grd.addColorStop(0, `rgba(${EMERALD},0.35)`);
          ...
          ctx.strokeStyle = `rgba(${EMERALD},${alpha * 0.5})`;
          ctx.lineWidth = 1;
          ctx.stroke();
          ctx.beginPath();
          ...
          ctx.beginPath();
          ctx.arc(p.x, p.y, r, 0, Math.PI * 2);
          ctx.fillStyle = `rgba(${EMERALD},${alpha})`;
          ctx.fill();
        } else {
          const isCyan = Math.sin(p.pulseOffset * 3.7) > 0.65;
          ctx.beginPath();
          ctx.arc(p.x, p.y, r, 0, Math.PI * 2);
